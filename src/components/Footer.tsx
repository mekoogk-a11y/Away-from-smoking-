...
export const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer className="mt-16 bg-slate-950 text-orange-100 border-t border-orange-900/60 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 pb-10 border-b border-slate-800">
          ...
          <div className="space-y-3 md:col-span-1">
            <AppLogo size="md" withText={true} lang={lang} />
            <p className="text-xs text-slate-400 leading-relaxed font-medium">
              {lang === 'ar'
                ? 'منصة عالمية متقدمة للتوعية بمخاطر التدخين وتقديم استشارات ومراكز معتمدة لمساعدة الملايين حول العالم على الإقلاع النهائي.'
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs font-bold text-slate-300">
              <li>
                <button onClick={() => onSelectTab('cravings')} className="hover:text-orange-400 transition-colors cursor-pointer">
                  {t.cravingRescue}
                </button>
              </li>
              <li>
                <button onClick={() => onSelectTab('videos')} className="hover:text-orange-400 transition-colors cursor-pointer">
                  {t.videoLibrary}
                </button>
              </li>
              <li>
                <button onClick={() => onSelectTab('journey')} className="hover:text-orange-400 transition-colors cursor-pointer">
                  {t.myJourney}
                </button>
              </li>
              <li>
                <button onClick={() => onSelectTab('sources')} className="hover:text-orange-400 transition-colors cursor-pointer">
                  {t.medicalSources}
                </button>
              </li>
              <li>
                <button onClick={() => onSelectTab('legal')} className="hover:text-orange-400 transition-colors cursor-pointer">
          ...
          {/* Col 4: 5 Languages & Direct Contact */}
          <div>
            <h4 className="font-extrabold text-sm text-white mb-3 tracking-wide">
              {t.switchLanguage}
            </h4>
        ...
        <div className="pt-8 text-center space-y-2">
          <p className="font-black text-sm sm:text-base text-amber-400">
            تصميم كمال جعفر زكريا
          </p>
          <p className="dir-ltr text-xs sm:text-sm font-bold text-orange-300">
